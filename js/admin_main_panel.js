...
//        }
    ...
    $("main .adm_form").submit(function() {
    	...
    	// age check
        var ageNode = $("main .adm_form input[name=age]");
        var ageInString = "";
        var age = 0;
        if(ageNode != null){
        	ageInString = $.trim($("main .adm_form input[name=age]").val());
        }
        var age_pattern=/[0-9]{0,3}/;
        	matchResult = ageInString.match(age_pattern);
    	if(matchResult != ageInString){
    		alert("Age format is not correct. It should only contain digits, and has a length of 0~3 digits!");
    		return false;
    	}        
        if(ageInString != null && ageInString != ""){
        	age = parseFloat(ageInString);
        }
        if(isNaN(age)==true){
        	alert("age should be a number");
        	$("main .adm_form input[name=age]").focus();
        	return false;
        }
        if(age<0 || age> 150){
        	alert("age is not valid, it should be between 0~150!");
        	$("main .adm_form input[name=age]").focus();
        	return false;
        }
